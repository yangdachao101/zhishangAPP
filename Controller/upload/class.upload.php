...
class upload {
     ...
     * @return boolean Success
     */
    function rmkdir($path, $mode = 7777) {
     ...
     * @return boolean Success
     */
    function _mkdir($path, $mode = 7777) {
        $old = umask(0);
        $res = @mkdir($path, $mode);
        umask($old);
        return $res;
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                //$this->file_dst_name_body = $this->file_dst_name_body;
                $this->file_dst_name_body = strtr($this->file_dst_name_body, array('脼' => 'TH', '镁' => 'th', '脨' => 'DH', '冒' => 'dh', '脽' => 'ss', '艗' => 'OE', '艙' => 'oe', '脝' => 'AE', '忙' => 'ae', '碌' => 'u'));
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($gd_version >= 2 && $this->image_unsharp && is_numeric($this->image_unsharp_amount) && is_numeric($this->image_unsharp_radius) && is_numeric($this->image_unsharp_threshold)) {
                        ...
                        // Unsharp mask algorithm by Torstein H酶nsi 2003-07.
